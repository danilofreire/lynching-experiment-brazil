...
packages <- c("cjoint", "cregg", "estimatr", "grf", "kableExtra", "janitor",
              "quanteda", "seededlda", "stargazer", "tidyverse")
...
We also note that there is another word cluster on the left. It contains words
...
Lastly, we estimate a latent Dirichlet allocation (LDA) model to identify the
...
stargazer(m1, m2, m3, m4, se = starprep(m1, m2, m3, m4),
          ...
          keep.stat = "n", no.space = TRUE, font.size = "footnotesize")
```
\normalsize
...
df_exp02_group <- df_exp02 %>%
  filter(gender %in% c("Female", "Male"),
         race %in% c("Asian", "Black", "Mixed Race", "White", "Indigenous"),
         education %in% c("College", "Graduate School", "Primary School",
  ...
  mutate(education2 = case_when(education == "Primary School" ~ "Primary or Secondary School",
         ...
         education2 = fct_relevel(education2, "Primary or Secondary School",
                                  "High School", "College", "Graduate School")) %>%
  mutate(views_police = fct_relevel(views_police, "Regular", "Very Good", "Good",
                                    "Bad", "Very Bad")) %>%
  mutate(views_justice = fct_relevel(views_justice, "Regular", "Very Good", "Good",
                                    "Bad", "Very Bad")) %>%
  mutate(ideology = fct_relevel(ideology, "Center", "Left", "Center-Left",
                                "Center-Right", "Right", "Don't Know",
                                "Rather Not Say")) %>%
  mutate(household_income2 = case_when(household_income == "Up to R$1,000" ~ "Up to R$3,000",
                                       household_income == "From R$1,001 to R$2,000" ~ "Up to R$3,000",
...
# Tables (robust standard errors from estimatr)
stargazer(m1, m2, m3, m4, m5, se = starprep(m1, m2, m3, m4, m5),
          p = starprep(m1, m2, m3, m4, m5, se_type = "HC1", stat = "p.value"),
          header = FALSE, align = TRUE, label = "tab:exp02demo1",
          title = "Experiment 02 -- Demographic Characteristics and Lynching Support",
          dep.var.labels = "\\textbf{Lynching Support}\\vspace{.5cm}", style = "apsr",
          column.labels = c("Gender", "Age", "Race", "Education", "Income"),
          covariate.labels = c("Male", "Years", "Black", "Mixed Race",
                               "Asian", "Indigenous", "High School", "College",
                               "Graduate School", "3,001 to 5,000 BRL", "5,000+ BRL"),
          column.sep.width = "2pt", notes = c("Reference categories: Model 1: Female;
                                              Model 3: White; Model 4: Primary or secondary school;",
                                              "Model 5: Up to 3,000 BRL. Robust standard errors in
                                              parentheses."),
          keep.stat = "n", no.space = TRUE, font.size = "footnotesize")
...
```{r exp02-demographics2, warning=F, message=F, error=F, cache=T, fig.pos="H", results='asis'}
stargazer(m6, m7, m8, m9, se = starprep(m6, m7, m8, m9),
          p = starprep(m6, m7, m8, m9, se_type = "HC2", stat = "p.value"),
          header = FALSE, align = TRUE, label = "tab:exp02demo2",
          title = "Experiment 02 -- Demographic Characteristics and Lynching Support",
          dep.var.labels = "\\textbf{Lynching Support}\\vspace{.5cm}", style = "apsr",
          column.labels = c("Ideology", "Previous Victimization", "Police",
                            "Judiciary"),
          covariate.labels = c("Left", "Center-Left", "Center-Right", "Right",
                               "Don't Know", "Rather Not Say", "Yes",
                               "Very Good", "Good", "Bad", "Very Bad",
                               "Don't Know", "Rather Not Say",
                               "Very Good", "Good", "Bad", "Very Bad",
                               "Don't Know", "Rather Not Say"),
          column.sep.width = "2pt", notes = c("Reference categories: Model 1: Center;
                                              Model 2: No; Model 3: Regular; Model 4: Regular.",
...
\footnotesize
```{r exp02-forest, warning=F, error=F, message=F, cache=T, fig.width=7, fig.height=6, eval=F}
df_exp02_police <- df_exp02_group %>%
  select(exp02_outcomes, exp02_police_treat, gender, age, race, education, household_income2, ideology, views_police, views_justice) %>%
  mutate(exp02_police_treat = as.numeric(exp02_police_treat))

  drop_na()

library(FindIt)
F1 <-FindIt(model.treat = exp02_outcomes ~ exp02_police_treat,
            model.main = ~ race,
            model.int = ~ race,
            data = df_exp02_police,
            type = "binary",
            treat.type = "single")

F1 <-FindIt(model.treat= outcome ~ treat,
            model.main= ~ age+educ+black+hisp+white+marr+nodegr+log.re75+u75,
            model.int= ~ age+educ+black+hisp+white+marr+nodegr+log.re75+u75, data = LaLonde, type="binary", treat.type="single")


cases <- sample(seq_len(nrow(df_exp02_police)), round(nrow(df_exp02_police) * .6))
train <- df_exp02_police[cases, ]
test <- df_exp02_police[-cases, ]

x <- c("age")
g <- c("gender", "race", "education", "household_income2",
"ideology", "views_police", "views_justice")
X2 <- model.matrix(~ 0 + ., train[c(x, g)])

cf <- causal_forest(
  X = X2,
  Y = as.numeric(train$exp02_outcomes),
  W = as.integer(train$exp02_police_treat),
  num.trees = 1000,
  seed = 1839
)
...
preds <- predict(
  object = cf,
  newdata = model.matrix(~ ., data = test[, 3:ncol(train)]),
  estimate.variance = TRUE
)

test$preds <- preds$predictions

ggplot(test, aes(x = views_justice, y = preds)) +
  geom_point() +
...
tree <- policy_tree(X2, dr_scores, 3)
plot(tree)
